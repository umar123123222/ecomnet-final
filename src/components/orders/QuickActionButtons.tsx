  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent align="end" className="bg-background z-50">
        <DropdownMenuItem onClick={() => onViewDetails(orderId)} className="gap-2">
          <FileText className="h-4 w-4" />
          View Details
        </DropdownMenuItem>
        
        {canDispatch && (
          <DropdownMenuItem onClick={() => onMarkDispatched(orderId)} className="gap-2">
            <CheckCircle className="h-4 w-4" />
            Mark as Dispatched
          </DropdownMenuItem>
        )}
        
        <DropdownMenuItem onClick={() => onGenerateLabel(orderId)} className="gap-2">
          <Truck className="h-4 w-4" />
          Generate Label
        </DropdownMenuItem>
        
        <DropdownMenuItem onClick={() => onViewActivity(orderId)} className="gap-2">

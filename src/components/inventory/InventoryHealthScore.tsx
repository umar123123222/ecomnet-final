...
export function InventoryHealthScore() {
  const { data: healthMetrics, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const stockAvailability = totalProducts > 0 
        ? ((inStockProducts - lowStockProducts * 0.5) / totalProducts) * 100 
        : 0;
      ...
      const { data: variances } = await supabase
        ...
        .select('variance_value, status')
        .gte('created_at', thirtyDaysAgo);

      const openVariances = variances?.filter(v => v.status === 'open').length || 0;
      const totalVariances = variances?.length || 0;
      const accuracyScore = totalVariances > 0 
        ? ((totalVariances - openVariances) / totalVariances) * 25
        : 25;
      ...
      const availableStock = inventory?.reduce((sum, inv) => 
        sum + (inv.quantity - (inv.product?.reorder_level || 0)), 0
      ) || 0;
      const fulfillmentCapacity = totalStock > 0 
        ? Math.max((availableStock / totalStock) * 100, 0)
        : 0;
  ...
  const getStatusIcon = (status: string) => {
    switch (status) {
      ...
      case 'fair': return <AlertTriangle className="h-6 w-6 text-orange-500" />;

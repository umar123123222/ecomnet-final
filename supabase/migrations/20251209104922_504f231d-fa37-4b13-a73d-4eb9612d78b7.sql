-- Drop ALL existing versions of rapid_dispatch_order function to eliminate overloading conflict
DROP FUNCTION IF EXISTS public.rapid_dispatch_order(text, uuid, uuid, text, text);
DROP FUNCTION IF EXISTS public.rapid_dispatch_order(text, text, uuid, text, text);
DROP FUNCTION IF EXISTS public.rapid_dispatch_order(p_entry text, p_user_id uuid, p_courier_id uuid, p_courier_name text, p_courier_code text);
DROP FUNCTION IF EXISTS public.rapid_dispatch_order(p_entry text, p_courier_id uuid, p_courier_name text, p_courier_code text, p_user_id uuid);

-- Recreate single clean version with consistent parameter order
...
BEGIN
  ...
  -- Deduct product inventory from main warehouse
  UPDATE inventory i
  SET 

-- Update handle_new_user function to check for supplier profiles and ensure supplier role
...
BEGIN
  ...
  -- Check if user has a supplier profile and ensure supplier role is active
  -- This must happen BEFORE determining primary_role to ensure supplier role is available
  PERFORM ensure_supplier_role(NEW.id);
  
  -- Re-fetch primary role after ensuring supplier role (if applicable)
  SELECT role INTO primary_role
  FROM user_roles
  WHERE user_id = NEW.id
    AND is_active = true
